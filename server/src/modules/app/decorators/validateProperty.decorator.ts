...
export function ValidateProperty(variable: string, isString: boolean, isNumber?: boolean, isPositive?: boolean): PropertyDecorator {
    return function (target: any, propertyKey: string) {
        ...
        if (isPositive) {
            decorators.push(IsPositive({ message: `${variable} must be a positive integer` }));
        }
