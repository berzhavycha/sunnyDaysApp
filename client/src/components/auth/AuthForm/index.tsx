...
export const AuthForm: FC<AuthFormProps> = ({ title, subTitle, fields, handleAuth, actionButtonText }) => {
  const { email, setEmail, password, setPassword, confirmPassword, setConfirmPassword, fieldsError } = fields;

  return (
    <View className="flex-1 justify-center items-center bg-gray-900">
      <Image source={require("@/assets/images/weather-icon.png")} className="w-[80px] h-[80px] mb-8" />
      <Text className={`text-2xl mb-${subTitle !== undefined ? "2" : "8"} text-white font-bold`}>{title}</Text>
      {subTitle && <Text className="text-xs mb-8 font-light text-gray-400">{subTitle}</Text>}
      <Input value={email} onChange={setEmail} placeholder="Email" icon="mail" error={fieldsError.email ?? ""} />
      <Input
        value={password}
