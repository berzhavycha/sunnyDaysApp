...
export const ForecastDay: React.FC<ForecastDayProps> = ({ text, dayOfWeek, tempCelsius, humidity }) => {
    ...
    return (
        <View className={`w-[${Math.floor(1 / REACT_APP_MAX_FORECAST_DAYS * 100 - 1)}%] justify-center items-center`}>
            <View className="w-full flex mb-2 justify-center items-center bg-blue-600 rounded-xl px-2 py-2">
                <Image source={{ uri: weatherIconMapping[dayWeatherIcon] }} style={{ width: 40, height: 40 }} className='mb-2 p-8' />
